...
typedef struct {
    ...
    XStft_input_uio_map maps[ MAX_UIO_MAPS ];
} XStft_input_uio_info;

/***************** Variable Definitions **************************************/
static XStft_input_uio_info uio_info;
...
static int uio_info_read_name(XStft_input_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XStft_input_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XStft_input_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XStft_input_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XStft_input_Initialize(XStft_input *InstancePtr, const char* InstanceName) {
	XStft_input_uio_info *InfoPtr = &uio_info;
