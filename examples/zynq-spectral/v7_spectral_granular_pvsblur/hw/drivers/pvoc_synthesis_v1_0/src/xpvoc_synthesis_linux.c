...
typedef struct {
    ...
    XPvoc_synthesis_uio_map maps[ MAX_UIO_MAPS ];
} XPvoc_synthesis_uio_info;

/***************** Variable Definitions **************************************/
static XPvoc_synthesis_uio_info uio_info;
...
static int uio_info_read_name(XPvoc_synthesis_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPvoc_synthesis_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPvoc_synthesis_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPvoc_synthesis_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPvoc_synthesis_Initialize(XPvoc_synthesis *InstancePtr, const char* InstanceName) {
	XPvoc_synthesis_uio_info *InfoPtr = &uio_info;
