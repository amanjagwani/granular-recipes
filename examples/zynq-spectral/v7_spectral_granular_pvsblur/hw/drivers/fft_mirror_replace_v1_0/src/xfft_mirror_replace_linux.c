...
typedef struct {
    ...
    XFft_mirror_replace_uio_map maps[ MAX_UIO_MAPS ];
} XFft_mirror_replace_uio_info;

/***************** Variable Definitions **************************************/
static XFft_mirror_replace_uio_info uio_info;
...
static int uio_info_read_name(XFft_mirror_replace_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFft_mirror_replace_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFft_mirror_replace_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFft_mirror_replace_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFft_mirror_replace_Initialize(XFft_mirror_replace *InstancePtr, const char* InstanceName) {
	XFft_mirror_replace_uio_info *InfoPtr = &uio_info;
