...
typedef struct {
    ...
    XOverlap_add_stft_uio_map maps[ MAX_UIO_MAPS ];
} XOverlap_add_stft_uio_info;

/***************** Variable Definitions **************************************/
static XOverlap_add_stft_uio_info uio_info;
...
static int uio_info_read_name(XOverlap_add_stft_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOverlap_add_stft_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOverlap_add_stft_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOverlap_add_stft_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOverlap_add_stft_Initialize(XOverlap_add_stft *InstancePtr, const char* InstanceName) {
	XOverlap_add_stft_uio_info *InfoPtr = &uio_info;
