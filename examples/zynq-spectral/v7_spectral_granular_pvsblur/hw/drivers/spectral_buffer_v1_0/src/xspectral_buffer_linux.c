...
typedef struct {
    ...
    XSpectral_buffer_uio_map maps[ MAX_UIO_MAPS ];
} XSpectral_buffer_uio_info;

/***************** Variable Definitions **************************************/
static XSpectral_buffer_uio_info uio_info;
...
static int uio_info_read_name(XSpectral_buffer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSpectral_buffer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSpectral_buffer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSpectral_buffer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSpectral_buffer_Initialize(XSpectral_buffer *InstancePtr, const char* InstanceName) {
	XSpectral_buffer_uio_info *InfoPtr = &uio_info;
